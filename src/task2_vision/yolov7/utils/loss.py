...
class ComputeLossOTA_multi:
    # Compute losses
    def __init__(self, model, autobalance=False):
        super(ComputeLossOTA_multi, self).__init__()
        device = next(model.parameters()).device  # get model device
        h = model.hyp  # hyperparameters
        ...
        '''
        BCEcls = nn.BCEWithLogitsLoss(pos_weight=torch.tensor([h['cls_pw']], device=device)) #tensor([1.],device='cuda:0')
        BCEobj = nn.BCEWithLogitsLoss(pos_weight=torch.tensor([h['obj_pw']], device=device)) #tensor([1.],device='cuda:0')
    ...
    def __call__(self, p, targets, imgs):  # predictions, targets, model   
        ...
        for i, pi in enumerate(p):  # layer index, layer predictions
            ...
            if n:
                ...
                # Classification, 첫번째 index인 class만 가져옴
                #tensor([3, 6, 6, 3, 0, 5, 6, 3, 0, 5, 3, 0, 3, 6, 7, 6], device='cuda:0')
                selected_tcls = targets[i][:, 1].long()
                if self.nc > 1:  # cls loss (only if multiple classes)
                    t = torch.full_like(ps[:, 5:], self.cn, device=device)  # targets
                    t[range(n), selected_tcls] = self.cp
                    #ps[:, 5:], torch.Size([16, 10])
                    #t, torch.Size([16, 10])
                    lcls += self.BCEcls(ps[:, 5:], t)  # BCE
                # Append targets to text file
                # with open('targets.txt', 'a') as file:
    ...
    def build_targets(self, p, targets, imgs):
        #targets의 첫번째 index는 image index
        #import pdb; pdb.set_trace()
...
class ComputeLossOTA:
    ...
    def build_targets(self, p, targets, imgs):
        ...
        nl = len(p)    
        #print(p[0].shape) : torch.Size([8, 3, 80, 80, 22])
        #import pdb; pdb.set_trace()
        for batch_idx in range(p[0].shape[0]):
            ...
            for i, pi in enumerate(p):
                #print(pi.shape) : torch.Size([8, 3, 80, 80, 22])
                #import pdb; pdb.set_trace()
                b, a, gj, gi = indices[i]
                idx = (b == batch_idx)
                b, a, gj, gi = b[idx], a[idx], gj[idx], gi[idx]                
                ...
                fg_pred = pi[b, a, gj, gi]  
                #print(fg_pred.shape) : torch.Size([9, 22]) 
        
                #print(fg_pred[:, 4:5].shape) : torch.Size([9, 1])
                #print(fg_pred[:, 5:].shape) : torch.Size([9, 17])
                #import pdb; pdb.set_trace()              
                p_obj.append(fg_pred[:, 4:5])
                p_cls.append(fg_pred[:, 5:])
    ...
    def find_3_positive(self, p, targets):
        #print(p[0].shape) : torch.Size([8, 3, 80, 80, 22])
        #print(targets[0]) : tensor([3.00000, 0.00000, 0.47286, 0.69657, 0.04076, 0.09346]
        #print(len(p)) : 3
        #import pdb; pdb.set_trace()
        ...
        ai = torch.arange(na, device=targets.device).float().view(na, 1).repeat(1, nt)  # same as .repeat_interleave(nt)
        targets = torch.cat((targets.repeat(na, 1, 1), ai[:, :, None]), 2)  # append anchor indices
        #print(targets.shape) torch.Size([3, 25, 7])
        #import pdb; pdb.set_trace()
        g = 0.5  # bias
        off = torch.tensor([[0, 0],
                            [1, 0], [0, 1], [-1, 0], [0, -1],  # j,k,l,m
        ...
        for i in range(self.nl):
            ...
            # [2.37500, 4.50000],
            # [5.00000, 3.50000]], device='cuda:1')
            # print(anchors.shape) torch.Size([3, 2])
            
            #print(p[i].shape) : torch.Size([8, 3, 80, 80, 22])
            #import pdb; pdb.set_trace()
            gain[2:6] = torch.tensor(p[i].shape)[[3, 2, 3, 2]]  # xyxy gain

            #print(gain[2:6]) : tensor([80, 80, 80, 80]
            #import pdb; pdb.set_trace()
            # Match targets to anchors
            t = targets * gain
            if nt:
...
class ComputeLossDrone:
    # Compute losses
    def __init__(self, model, autobalance=False):
        super(ComputeLossDrone, self).__init__()
        device = next(model.parameters()).device  # get model device
        h = model.hyp  # hyperparameters
